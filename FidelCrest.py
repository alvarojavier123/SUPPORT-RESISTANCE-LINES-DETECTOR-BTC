...
def INTRADAY_ZOOM_1Hr(trend, structures, agent_date):
    ...
    agent_date = agent_date + dt.timedelta(hours=1)
...
def Trade(asset, position, entry_time, structures, limites, entry_area, TP, SL, trailing):
    ...
    if position == 'LONG':
            ...
            stop_loss = entry_price - (entry_price * SL)
            print('Inital stop loss =', stop_loss)

            trailing_stop_threshold = entry_price + (entry_price * trailing)
    ...
    for i in hours.index:
            ...
            if position == 'LONG':
                take_profit = entry_price + (entry_price * TP)
                ...
                if pnl > 0:
                        ...
                        if c >= trailing_stop_threshold and trailing_stop == False:
                              ...
                              trailing_stop = True
                              stop_loss = c - (c * trailing)
                              print(Fore.LIGHTYELLOW_EX  + 'TRAILING STOP FOLLOWED '+ Style.RESET_ALL)
                              print(Fore.LIGHTYELLOW_EX  + 'TRAILING STOP = ', str(trailing_stop) + Style.RESET_ALL)
                              print(Fore.LIGHTYELLOW_EX  + 'STOP LOSS AT = ', str(stop_loss) + Style.RESET_ALL)
                              continue
...
for asset in assets:
    ...
    for day in range(0, len(daily)):
        ...
        if date >= dt.datetime(2023, 1, 1, 19, 0, 0):
            ...
            if position == True:
                  if trend == 'BEAR':
                        entry_time, exit_time = Trade(
                              asset, 'SHORT', exit_time, structures, boundaries, 
                              entry_area, TP, SL, trailing_stop
                              )
                        position = False

                  elif trend == 'BULL':
                        entry_time, exit_time = Trade(
                              asset, 'LONG', exit_time, structures, boundaries, 
                              entry_area, TP, SL, trailing_stop
                              )
                        position = False
