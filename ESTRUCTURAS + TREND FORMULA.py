...
def Trade(asset, position, entry_time, structures , limites):    
      ...
      hours = hours.loc[entry_time:entry_time + dt.timedelta(days=3)]
      print(hours)
...
for asset in assets:
    ...
    for day in range(0, len(daily)):
        ...
        if date >= dt.datetime(2020, 1, 1, 19, 0, 0):
            ...
            # Convert exit_time if it's not already a datetime
            if isinstance(exit_time, int):
                  exit_time = datetime.fromtimestamp(exit_time)

            cutoff_today = exit_time.replace(hour=19, minute=0, second=0)
            cutoff_next = (exit_time + timedelta(days=1)).replace(hour=19, minute=0, second=0)

            if (
            exit_time < cutoff_today or
            (cutoff_today < exit_time < cutoff_next)
            ):
