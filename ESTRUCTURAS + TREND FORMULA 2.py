...
def Trade(asset, position, entry_time, structures , limites):    
      ...
      for i in hours.index:
            ...
            if position == 'SHORT' and close_to_area == False:
                  ...
                  if pnl > 0:
                        ...
                        if returns > 0.05:
                              ...
                              stop_loss = c + (c * 0.03)
                              print(Fore.LIGHTYELLOW_EX  + 'STOP LOSS TRIGGER = ', str(stop_loss_trigger) + Style.RESET_ALL)
            ...
            for date, area in structures.items():
                  ...
                  #print('Area = ', area[0])
                    ...
                    if (
                        (o < upper_zone and c > lower_zone) or 
                        (h < upper_zone and c > lower_zone) or
                        (l < upper_zone and c > lower_zone) or
                        (c < upper_zone and c > lower_zone)
                        ):
            ...
            if close_to_area == True:
                  if position == 'SHORT':
                        ...
                        returns = pd.Series([boundaries['upper zone'], entry_price])
                        returns = round(returns.pct_change()[1], 4)
                        returns = returns - (returns * 0.0007)
                        ...
                        trades.loc[time] = {
                              ...
                              'exit time' : time,
                              'entry price' : entry_price,
                              'exit price' : boundaries['upper zone'],
                              'position' : position
                              }
                        ...
                        print(Fore.LIGHTRED_EX  + 'Price Close To Area' + Style.RESET_ALL)
                        returns = pd.Series([entry_price, statistics.mean([boundaries['upper zone'], boundaries['lower zone']])])
                        returns = round(returns.pct_change()[1], 4)  
                        returns = returns - (returns * 0.0007)
                        ...
                        trades.loc[time] = {
                              ...
                              'exit time' : time,
                              'entry price' : entry_price,
                              'exit price' : statistics.mean([boundaries['upper zone'], boundaries['lower zone']]),
                              'position' : position
                              }
                  ...
                  (close_to_area == False) and
                  (OPEN_POSITION == True) and 
                  (position == 'LONG') and 
                  (o < limites['lower zone'] or 
                   h < limites['lower zone'] or
                   l < limites['lower zone'] or 
                   c < limites['lower zone'])
                  ):
      
                  returns = pd.Series([entry_price, statistics.mean([limites['upper zone'], limites['lower zone']])])
                  returns = round(returns.pct_change()[1], 4)
                  returns = returns - (returns * 0.0007)
                  ...
                  trades.loc[time] = {
                              ...
                              'exit time' : time,
                              'entry price' : entry_price,
                              'exit price' : statistics.mean([limites['upper zone'], limites['lower zone']]),
                              'position' : position
                              }
                  ...
                  OPEN_POSITION = False
                  print(Fore.LIGHTYELLOW_EX  + 'OPEN POSITION = ' + str(OPEN_POSITION) + Style.RESET_ALL)
                  return entry_time, time, statistics.mean([limites['upper zone'], limites['lower zone']])

            elif (
                  (close_to_area == False) and
                  (OPEN_POSITION == True) and 
                  (position == 'SHORT') and 
                  (o > limites['upper zone'] or 
                   h > limites['upper zone'] or
                   l > limites['upper zone'] or 
                   c > limites['upper zone'])
                  ):
      
                  returns = pd.Series([statistics.mean([limites['upper zone'], limites['lower zone']]), entry_price])
                  returns = round(returns.pct_change()[1], 4)
                  returns = returns - (returns * 0.0007)
      ...
      if position == 'SHORT':
            ...
            print('Trades = ' , trades)
            return entry_time, hours.iloc[-1].name, AREA
...
def zoom1Hr(trend, structures, agent_date):
    ...
    for i in hours.index:
            ...
            if close_to_area == True:
                  if trend == 'BEAR':
                        ...
                        if (
                            (c >= boundaries['lower zone'] and c <= AREA) and
                            ...
                            marubozu == True
                              ...
                              return short, exit_time, boundaries
                        ...
                        print('Boundaries = ', boundaries)
                        if (
                            (c <= boundaries['upper zone'] and c >= AREA) and
                            ...
                            marubozu == True    
                              ...
                              return long, exit_time, boundaries
    ...
    if short == True:
        ...
        return False, exit_time, boundaries
...
for asset in assets:
    ...
    for day in range(0, len(daily)):
        ...
        if date == dt.datetime(2021, 1, 1, 19, 0, 0):
            break

        if date >= dt.datetime(2020, 2, 24, 19, 0, 0):
            ...
            if trend == 'BULL' and date >= exit_time:
                  position, exit_time, boundaries = zoom1Hr(trend, structures, date)
            ...
            if position == True:
                  if trend == 'BEAR':
                        entry_time, exit_time, last_area = Trade(asset, 'SHORT', exit_time, structures, boundaries)
                        position = False

                  elif trend == 'BULL':
                        entry_time, exit_time, last_area = Trade(asset, 'LONG', exit_time, structures, boundaries)
                        position = False
            ...
            if (
                  ...
                  position, exit_time, boundaries = INTRADAY_ZOOM_1Hr(trend, structures, exit_time)
                  if position == True:
                        if trend == 'BEAR':
                              entry_time, exit_time, last_area = Trade(asset, 'SHORT', exit_time, structures, boundaries)
                              position = False
